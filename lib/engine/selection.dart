...
class SelectionEngine {
  ...
  List<Map<String, dynamic>> _inZone(
    List<Map<String, dynamic>> states,
    String zone,
    int questionIndex,
  ) {
    // pickZone 的第二个参数必须是"当前第几道题"（跟设计文档 pick_zone(mastery,
    // question_index) 一致），而不是 kp_id——否则同一个知识点在同一个 mastery
    // 值下会永远被分到同一个区，四区调度的"软分区+随机性"就名存实亡了。
    // 每个知识点仍然用 kp_id 做一点扰动，避免同一批次里所有知识点结果完全一样。
    return states.where((s) {
      final mastery = s['effective_mastery'] as double;
      final seed = questionIndex * 1000003 + (s['kp_id'] as int);
      return pickZone(mastery, seed) == zone;
    }).toList();
  }
